...
class LaPampaCasesTest extends TestCase
{
    use FactoriesTrait;
    use ApiAuthenticationTrait;
    use WithholdingMocks;

    private const LA_PAMPA = 42;
    ...
    private const LA_PAMPA_INSCRIPTO_LOCAL_RATE = 2.5;
    private const LA_PAMPA_INSCRIPTO_CM_RATE = 2.5;
    private const LA_PAMPA_HABITUALIDAD_RATE = 2.5;

    private const LA_PAMPA_JURISDICTION = '911';

    /** @var EntityManagerInterface */
    private $entityManager;

    /** @var Province */
    private $laPampaProvince;
    ...
    public function setUp(): void
    {
        ...
        $this->laPampaProvince = $this->entityManager->getRepository(Province::class)->find(self::LA_PAMPA);
    }
    ...
    {
        $baseAmount = 500;
        $this->buildRuleTestScenario($this->laPampaProvince, self::INSCRIPTO_LOCAL, 1, $baseAmount);
        ...
        $withholdedAmount = round($taxableIncome * (self::LA_PAMPA_INSCRIPTO_LOCAL_RATE / 100), 2);
    ...
    public function has_inscripto_cm_rule(): void
    {
        ...
        $this->buildRuleTestScenario($this->laPampaProvince, self::INSCRIPTO_CONVENIO_MULTILATERAL, 1, $baseAmount);

        /** @var WithholdingTaxDetail[] $withholdingTaxDetails */
        $withholdingTaxDetails = $this->entityManager->getRepository(WithholdingTaxDetail::class)->findAll();

        $taxableIncome = $baseAmount;
        $withholdedAmount = round($taxableIncome * (self::LA_PAMPA_INSCRIPTO_CM_RATE / 100), 2);
    ...
    public function has_exento_exclusion_rule(): void
    {
        ...
        $saleBag = $this->buildRuleTestScenario($this->laPampaProvince, self::EXENTO, 1, $baseAmount, $commission,
            $commissionTax);
    ...
    public function has_regimen_simplificado_rule(): void
    {
        ...
        $saleBag = $this->buildRuleTestScenario($this->laPampaProvince, self::REGIMEN_SIMPLIFICADO,
    ...
    /** @test */
    public function habituality_with_less_than_3_trx_doesnt_withhold(): void
    {
        $transactionCount = 2;
        $baseAmount = 3700;
        $commission = 0.1;
        ...
        $saleBag = $this->buildRuleTestScenario($this->laPampaProvince, self::NO_INSCRIPTO,
    ...
    /** @test */
    public function habituality_with_3_or_more_trx_does_withhold(): void
    {
        $transactionCount = 3;
        $baseAmount = 4000;
        $commission = 0.1;
        ...
        $commissionAmount = round($baseAmount * $commission, 2);
        $commissionTaxAmount = round($commissionAmount * $commissionTax, 2);
        $taxWithheld = round($baseAmount * self::LA_PAMPA_HABITUALIDAD_RATE / 100, 2);
